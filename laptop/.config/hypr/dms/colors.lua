-- Auto-generated by DMS Matugen hook — do not edit manually.
-- Remove require("dms.colors") from hyprland.lua to override.

hl.config({
	general = {
		col = {
			active_border = "rgb(ebc06c)",
			inactive_border = "rgb(9a8f80)",
		},
	},
	group = {
		col = {
			border_active = "rgb(ebc06c)",
			border_inactive = "rgb(9a8f80)",
			border_locked_active = "rgb(ffb4ab)",
			border_locked_inactive = "rgb(9a8f80)",
		},
		groupbar = {
			col = {
				active = "rgb(ebc06c)",
				inactive = "rgb(9a8f80)",
				locked_active = "rgb(ffb4ab)",
				locked_inactive = "rgb(9a8f80)",
			},
		},
	},
})
